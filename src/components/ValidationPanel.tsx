...
function SideStatus({ label, side }: { label: string; side: SideRunResult }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <span style={{ color: 'var(--text-dim)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase' }}>{label}</span>
      <span style={{ color: ok ? 'var(--color-green)' : 'var(--color-error)', fontSize: 11 }}>
        {side.loaded ? 'Loaded' : 'Load failed'} / {side.inferenceOk ? 'Inference OK' : 'Inference failed'}
      </span>
      {side.error && <span style={{ color: 'var(--color-error)', fontSize: 10, overflowWrap: 'anywhere' }}>{side.error}</span>}
    </div>
  )
}
...
export function ValidationPanel({
  ...
  if (isReadOnly) {
    return (
      <section aria-label="Behavioral validation" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, color: 'var(--text-dim)', fontFamily: 'var(--font-mono)', fontSize: 11, textAlign: 'center' }}>
  ...
  return (
    ...
    >
      <div style={{ padding: '12px 16px 8px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {providedInputNames && providedInputNames.length > 0 && (
            <button type="button" data-testid="validate-clear-inputs" onClick={onClearProvidedInputs} className="btn-ghost" style={{ fontSize: 10, padding: '2px 8px' }}>
              Clear ({providedInputNames.join(', ')})
            </button>
          )}
        </div>
        {fileError && <span style={{ color: 'var(--color-error)', fontSize: 10 }}>{fileError}</span>}
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
        {!currentResult ? (
          <span style={{ color: 'var(--text-dim)', fontSize: 11 }}>Not yet run for this edit state.</span>
        ) : (
          <>
      ...
      {validatedStates.length > 0 && (
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.08)', padding: '8px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          <span style={{ color: 'var(--text-dim)', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Validated edit states</span>
